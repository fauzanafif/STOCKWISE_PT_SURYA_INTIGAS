...
from stockwise.config import DEFICIT_WEIGHT, LEAD_TIME_HIGH_THRESHOLD_DAYS, LEAD_TIME_WEIGHT
...
def run_calc(threshold: float | None = None, notes: str = "app") -> dict:
    ...
    try:
        ...
        items = conn.execute(
            """
            SELECT m.id,
                   s.sisa_stok_num  AS sisa, s.sisa_stok_known AS sisa_known,
                   ssp.safety_stock AS ss, ssp.lead_time_days AS lt, ssp.avg_12_bln AS avg12
            FROM master_items m

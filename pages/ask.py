...
from stockwise.ui import fmt_num, page_header, require_db, status_badge

page_header("Ask STOCKWISE", "Pertanyaan yang sering muncul di rapat — jawaban langsung dari data.", icon="💬")
if not require_db():
    st.stop()

mode = st.radio("", ["📋 Pertanyaan umum", "🔎 Tanya satu barang"], horizontal=True, label_visibility="collapsed")
st.divider()
...
if mode.startswith("🔎"):
    ...
    st.page_link("pages/item_detail.py", label="Buka Item Detail lengkap →", icon="🔍")

...
from utils.sw_config import ROOT, STATUS_COLORS, STATUS_LABELS
...
def page_header(title: str, subtitle: str = "", icon: str = ""):
    """First call on every page in pages/: page config + shared CSS + sidebar
    brand + page title. `app.py` (the main page) keeps its own chrome."""
    try:
...
def require_db() -> bool:
    if db_exists():
        return True
    st.warning(
        "**Data belum ada.** Upload file master di halaman utama (**app**), atau semua file "
        "sekaligus di **Kelola Data**. Alternatif CLI: "
        "`node --experimental-sqlite tools/build_stockwise_db.mjs` (mengisi dari `DATAFIX/`)."
    )
    return False
...
def setup_status() -> dict:
    ...
    from utils.database import scalar
...
def attention_strip():
    ...
    bits = []
    if s["ss_open"]:
        bits.append((f"🛡️ {s['ss_open']:,} konflik Safety Stock", "pages/6_🛡️_Safety_Stock.py"))
    if s["match_open"]:
        bits.append((f"🤝 {s['match_open']:,} barang perlu di-match", "pages/5_🤝_Cocokkan_Barang.py"))
    if not bits:
        return
